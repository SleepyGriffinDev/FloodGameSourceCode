...
public class AnaJBNewMovementBehaviour : MonoBehaviour
{
    ...
    float jumpCDTimerFull = 0.5f;
    float groundCheckTimer = 0f;
    float groundCheckTimerFull = 0.5f;
    ...
    float rayCastDistance = 0.45f;
    float diagRayCastDistance = 0.31820f;
    ...
    /*For controlling the "DirectionFaced" Animator Parameter*/
    ...
    void FixedUpdate()
    {
        ...
        if (rb.velocity.magnitude != 0 && isGrounded && !playingFootsteps)
        {
            StartFootsteps();
        }
        else if (!isGrounded || (rb.velocity.magnitude > -0.1f && rb.velocity.magnitude < 0.1f))
        {
            StopFootsteps();
        }
